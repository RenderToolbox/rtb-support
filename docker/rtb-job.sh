...
function copy_data ()
{
  ...
  then
    echo "Copy <$SOURCE> to bucket <$DESTINATION>"
...
echo "Create <$INPUT_SCRATCH> and <$OUTPUT_SCRATCH>"
...
MATLAB_COMMAND="try; run('$STARTUP'); if exist('tbUse', 'file'); tbUse($TOOLBOXES); end; $COMMAND; catch err; disp(err); save(fullfile('$OUTPUT_SCRATCH', sprintf('rtb-job-%s', datestr(now(), 30)))); end; exit();"
